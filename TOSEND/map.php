...
$member_info_query = "SELECT CONCAT(F_NAME,' ',L_NAME) AS NAME, 
						IFNULL(TOWN, 'NONE') AS TOWN, IFNULL(STATE, 'NONE') AS STATE, IFNULL(LAT, '') AS LAT, IFNULL(LNG, '') AS LNG
						FROM MEMBERS";
			...
			<div id="member-output" class="col-sm-4">
				...
				<?PHP
					...
					foreach ($memberInfoArray as $key => $value) {
						...
						print "<td class='memberLat'>".$value['LAT']."</td>";
						print "<td class='memberLng'>".$value['LNG']."</td>";
						print "</tr>";
					}
					print "</table>";
				?>
